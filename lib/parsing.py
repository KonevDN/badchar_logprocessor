    ...
    for index in range(len(array_of_current_strings)):
        ...
        if (objMatch != None):
            ...
            if str_found_command_name in dict_of_atcommands:
                str_help_string = array_of_current_strings[index] + ' '*20 + 'help: ' + dict_of_atcommands[str_found_command_name] # составляется строка с подсказкой о назначении АТ-команды взамен текущей строки массива
